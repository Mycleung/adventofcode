...
object Day2 extends App {
  def diff(line: List[Int]): Int = {
    val (min, max) = line.foldLeft[(Int, Int)]((-1, -1)) { (acc, next) =>
      val min = if (acc._1 == -1 || acc._1 > next) next else acc._1
      val max = if (acc._2 == -1 || acc._2 < next) next else acc._2
      (min, max)
    }
    println(s"Min: $min, Max: $max, Diff: ${max - min}")
    max - min
  }

  def div(line: List[Int]): Int = {
    @tailrec
    def f(next: Int, rest: List[Int]): Int = {
      val maybe = rest.foldLeft(0)((acc, b) => if (b % next == 0) b / next else if (next % b == 0) next / b else acc)
      if (maybe != 0) maybe else f(rest.head, rest.tail)
    }
    f(line.head, line.tail)
  }

  def checksum(line: List[List[Int]], agg: List[Int] => Int): Int = {
    line.foldLeft[Int](0)((acc, next) => acc + agg(next))
  }

  def parse(in: String): List[List[Int]] = {
    val lines = in.split("\n")
    lines.map(_.split("	").map(_.toInt).toList).toList
  }

  def solve1(input: String): Int = {
    checksum(parse(input), diff)
  }

  def solve2(input: String): Int = {
    checksum(parse(input), div)
  }

  val puzzle = """157	564	120	495	194	520	510	618	244	443	471	473	612	149	506	138
                 ...
                 |411	178	4263	4690	3653	162	3201	4702	3129	2685	3716	147	3790	4888	79	165""".stripMargin

  println(s"Part 1 solution is: ${solve1(puzzle)}")
  println(s"Part 2 solution is ${solve2(puzzle)}")
}
